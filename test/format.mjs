...
const lessonNumbers = ['1.1', '1.2', '1.3', '1.4', '1.5', '1.6', '2.1'];
...
async function main() {
	...
	for (const cl of charsList) {
		...
		const title = `Level ${lessonNumber}: ${newChars.toUpperCase()}`;
		...
		await fs.promises.writeFile(`../src/lib/data/lessons/${lessonNumber}.json`, JSON.stringify(format, null, 2));
		console.log(
			`Added ${wordsSet.size} words. Sample: ${wordsInfo
				.slice(0, 5)
				.map((w) => w.word)
				.join(', ')}`
		);
